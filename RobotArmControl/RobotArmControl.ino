...
//void doAutoMove() {
  ...
  //BasePivotServo.easeTo(90);
  //delay(1000);
  //BasePivotServo.easeTo(45);
  //setPivotServos(135,45,135,45);
  //return;
//}
//#define TRACE
//#define DEBUG
 ...
 */
//THIS IS WHERE THE MAIN PART OF THE CODE BEGINS. 
...
void handleManualControl() {
    ...
    if (millis() - sLastMillisOfManualCheck >= MANUAL_CHECK_PERIOD_MILLIS) {
        sLastMillisOfManualCheck = millis(); //checks the amount of time that has passed since the program has started running 
...
// reset manual action after the first move to manual start position
	    ...
	    while(!Serial.available());
            sFirstPivot = sLastPivot = Serial.readString().toInt();//analogRead(PIVOT_INPUT_PIN);
	...
	while(!Serial.available());
        int tPivot = Serial.readString().toInt();//analogRead(PIVOT_INPUT_PIN);
